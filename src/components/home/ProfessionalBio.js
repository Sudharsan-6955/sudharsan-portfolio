...
export default function ProfessionalBio() {
    ...
    const handleMakeItFly = () => {
        if (isLaunching) return;
        setIsLaunching(true);

        // Allow the rocket launch blast-off animation to play, then open the resume modal
        setTimeout(() => {
            setIsModalOpen(true);
                ...
                <div className="relative z-10 flex flex-col gap-12 ">
                    ...
                    <div className="flex flex-col p-3 ">
                        ...
                        <div className="flex justify-between  items-center md:pt-9 md:pb-4">
                            ...
                            <button
                                onClick={handleMakeItFly}
                                className="group ml-6 font-syne gold-text bg-gray-800 hover:bg-gray-700 active:scale-95 text-white px-3 md:px-4 py-3 rounded-full text-sm font-medium transition-all flex items-center gap-2 whitespace-nowrap cursor-pointer relative overflow-visible shadow-md hover:shadow-orange-500/10"
                                aria-label="Make it fly - Open Resume"
                                title="Click to launch Resume"
                            >
                                Make it fly!
                                <span className="rocket-container">
                                    <Image
                                        ...
                                        alt="rocket"
                                        className={`rocket-icon md:h-5 h-4 w-auto ${isLaunching ? 'launching' : ''}`}
                                        width={20}
                                        height={20}
                                    />
                                    {/* Back side Turbo Pure Air Smoke */}
                                    <span className={`rocket-exhaust ${isLaunching ? 'launching' : ''}`} aria-hidden="true">
